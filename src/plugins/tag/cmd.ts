...
export default CreateCommand({
	...
	run: async (instance, interaction) => {
		...
		if (subCommand.find((name) => name === 'create')) await handleCreate(instance, interaction);
		if (subCommand.find((name) => name === 'delete')) await handleDelete(instance, interaction);

		if (subCommand.find((name) => name === 'edit')) await handleEdit(instance, interaction);

		if (subCommand.find((name) => name === 'list')) await handleList(instance, interaction);

		if (subCommand.find((name) => name === 'view')) await handleView(instance, interaction);

		if (subCommand.find((name) => name === 'clear')) await handleClear(instance, interaction);
